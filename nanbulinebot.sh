...
diff .nanbulinebot-now .nanbulinebot-last > /dev/null || (cat .nanbulinebot-now && ./tweet.sh post < .nanbulinebot-now && psql $DATABASE_URL --command="INSERT INTO status VALUES (DEFAULT, 'test', '$(cat .nanbulinebot-now)') ON CONFLICT (bot) DO UPDATE SET timestamp = DEFAULT, message = '$(cat .nanbulinebot-now)';")
